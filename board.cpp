...
void ChessBoard::execute_pseudo_move(const Move move, int moving_bb, int target_bb) {
...
void ChessBoard::generate_pseudo_moves(MoveList& list) const {
    ...
    while(our_pieces) {
        ...
        while (piece_moves) {
            ...
            bool is_promotion = (bb_index == 0 || bb_index == 6) && (target / 8 == 7 || target / 8 == 0);

            bool is_ep = (bb_index == 0 || bb_index == 6) && 
                        en_passant_square != -1 && 
                        target == en_passant_square + (white_turn ? 8 : -8) &&
                        abs(square % 8 - en_passant_square % 8) == 1;

            bool is_castling = (bb_index == 5 || bb_index == 11) && abs(square - target) == 2;
...
uint64_t ChessBoard::generate_knight_moves_from(int square, bool white) const {
    uint64_t valid_targets = AttackTables::knight_attacks[square];
...
uint64_t ChessBoard::generate_bishop_moves_from(int square, bool white) const {
    uint64_t mask = AttackTables::bishop_masks[square];
    uint64_t blockers = get_occupancy() & mask;
    uint64_t index = (blockers * AttackTables::bishop_magics[square]) >> (64 - AttackTables::bishop_shifts[square]);

    uint64_t valid_targets = AttackTables::bishop_attacks[square][index];
    uint64_t our_pieces = white ? get_white_pieces() : get_black_pieces();
    valid_targets &= ~our_pieces;
    return valid_targets;
}
uint64_t ChessBoard::generate_rook_moves_from(int square, bool white) const {
    uint64_t mask = AttackTables::rook_masks[square];
    uint64_t blockers = get_occupancy() & mask;
    uint64_t index = (blockers * AttackTables::rook_magics[square]) >> (64 - AttackTables::rook_shifts[square]);

    uint64_t valid_targets = AttackTables::rook_attacks[square][index];
    uint64_t our_pieces = white ? get_white_pieces() : get_black_pieces();
    valid_targets &= ~our_pieces;
    return valid_targets;
}
...
bool ChessBoard::is_square_attacked(int square, bool by_white) const {
    ...
    uint64_t bishop_mask = AttackTables::bishop_masks[square];
    uint64_t bishop_blockers = occupied & bishop_mask;
    uint64_t bishop_index = (bishop_blockers * AttackTables::bishop_magics[square]) >> (64 - AttackTables::bishop_shifts[square]);
    uint64_t bishop_valid_targets = AttackTables::bishop_attacks[square][bishop_index];

    if (bishop_valid_targets & (get_bishops(by_white) | get_queens(by_white))) return true;

    uint64_t rook_mask = AttackTables::rook_masks[square];
    uint64_t rook_blockers = occupied & rook_mask;
    uint64_t rook_index = (rook_blockers * AttackTables::rook_magics[square]) >> (64 - AttackTables::rook_shifts[square]);
    uint64_t rook_valid_targets = AttackTables::rook_attacks[square][rook_index];
