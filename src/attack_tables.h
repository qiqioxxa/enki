...
namespace AttackTables {
    ...
    namespace detail {
        extern std::array<std::array<uint64_t, 64>, 2> pawn_pushes_;
        extern std::array<std::array<uint64_t, 64>, 2> pawn_attacks_;
        extern std::array<uint64_t, 64> knight_attacks_;
        extern std::array<uint64_t, 64> king_attacks_;
        extern std::array<std::array<uint64_t, 512>, 64> bishop_attacks_;
        extern std::array<std::array<uint64_t, 4096>, 64> rook_attacks_;
        ...
        constexpr std::array<uint64_t, 64> bishop_masks = []() {
        ...
        constexpr std::array<uint64_t, 64> rook_masks = []() {
            std::array<uint64_t, 64> masks{};
            for (int square = 0; square < 64; square++) {
                int rank = square / 8, file = square % 8;
                uint64_t mask = 0;            
        ...
        constexpr std::array<std::array<uint64_t, 64>, 64> line_bb_ = []() {
        ...
        constexpr std::array<std::array<uint64_t, 64>, 64> between_bb_ = []() {
    ...
    inline uint64_t pawn_pushes(int square, bool white) {
        return detail::pawn_pushes_[!white][square];
    }
    inline uint64_t pawn_attacks(int square, bool white) {
        return detail::pawn_attacks_[!white][square];
    }
    inline uint64_t knight_attacks(int square) {
        return detail::knight_attacks_[square];
    }
    inline uint64_t king_attacks(int square) {
        return detail::king_attacks_[square];
    }
    inline uint64_t bishop_attacks(int square, uint64_t occupancy) {
        ...
        uint64_t index = (blockers * detail::bishop_magics[square]) >> (64 - detail::bishop_shifts[square]);
        return detail::bishop_attacks_[square][index];
    }
    inline uint64_t rook_attacks(int square, uint64_t occupancy) {
        uint64_t mask = detail::rook_masks[square];
        uint64_t blockers = occupancy & mask;
        uint64_t index = (blockers * detail::rook_magics[square]) >> (64 - detail::rook_shifts[square]);
        return detail::rook_attacks_[square][index];
    }

    inline uint64_t line_bb(int square1, int square2) {
        return detail::line_bb_[square1][square2];
    }
    inline uint64_t between_bb(int square1, int square2) {
        return detail::between_bb_[square1][square2];
    }
};
